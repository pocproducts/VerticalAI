...
> **Delta from** `openspec/specs/rate-limiting/spec.md`. This spec replaces the
> in-memory fixed-window design with Redis-backed sliding windows and an ASGI
...
If no plan is resolved for the API key, the rate limiter MUST fall back to 10
RPM and 100 RPD defaults.
