...
> **New capability.** This spec defines the `Tenant` data model, the
...
1. If `ApiKey` model gains a `tenant_id` field → use it directly
...
#### Scenario: Tenant resolved from developer binding

- GIVEN an authenticated developer with `id="dev_01"` and a binding to
  Tenant `tnt_01`
...
- GIVEN an authenticated developer with NO tenant binding
- WHEN the TenantContext middleware runs
...
- GIVEN a request that failed auth (no `request.state.developer`)
- WHEN the TenantContext middleware runs
