...
> **Post-verification changes applied 2026-06-12**:
> 1. **Error propagation**: browser errors (timeout, etc.) are now set in `resultado['error']` so the report shows `❌` instead of false `✅`. Affects `_procesar_cliente_pipeline()` in cli.py.
> 2. **Partial data loss on timeout**: `_run_single()` now consolidates successful tasks before returning error, so data from `full` is preserved even if `facilidades` times out. Affects `composio.py` error handlers.
> 3. **Timeout increase**: `FacilidadesTask.timeout` increased from 600s to 900s. Affects `task.py`.
> 4. **Final message**: `report()` now differentiates between full success (`✅`), error (`❌`), and incomplete (`⚠️`). Affects `cli.py`.
> 5. **Task stats display**: each task now shows `⏱️ X.Xs | 👣 N steps | Plan: Estudio Contable | 💰 $0` at completion. Affects `composio.py`.
> 6. **Live URL always visible**: browser URL shown at task launch (removed `--headed` guard). Affects `composio.py`.
> 7. **None-safe parsing**: fixed `float(None)` and `int(None)` crashes in `_parse_facilidad()` and `_consolidate()` when the AI agent returns `null` for `capital`, `interes_*`, `anticuota`, `saldo`. Plans and debts that were silently dropped now parse correctly. Affects `composio.py`.
