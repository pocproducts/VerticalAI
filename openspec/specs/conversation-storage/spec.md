...
The system MUST provide `GET /v1/chat/conversations` that returns conversations for the authenticated tenant. Each entry MUST include `conversation_id`, `title` (first message preview), `message_count`, and `updated_at`. Results MUST be ordered by `updated_at` descending.
...
Conversations MUST be stored as Redis hashes with key prefix `tenant:{tenant_id}:conv:{conversation_id}`. Each hash MUST contain `messages` (JSON array), `created_at`, and `updated_at` (ISO timestamps).
...
- AND it MUST be a hash with `messages`, `created_at`, `updated_at`
