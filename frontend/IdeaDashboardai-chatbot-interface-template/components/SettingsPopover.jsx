"use client"
import { useState } from "react"
import Link from "next/link"
import { useUser, useClerk } from "@clerk/nextjs"
import { Settings, Key, Crown, LogOut, ChevronRight, User } from "lucide-react"
import { Popover, PopoverContent, PopoverTrigger } from "./ui/popover"
import { Avatar, AvatarImage, AvatarFallback } from "./ui/avatar"

export default function SettingsPopover({ children }) {
  const [open, setOpen] = useState(false)
  const { user, isLoaded } = useUser()
  const { signOut } = useClerk()

  const initials = isLoaded && user
    ? (user.fullName || user.emailAddresses?.[0]?.emailAddress || "U").slice(0, 2).toUpperCase()
    : "?"

  return (
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>{children}</PopoverTrigger>
      <PopoverContent className="w-72 p-0" align="start" side="top">
        <div className="p-3">
          {/* Email real desde Clerk */}
          <div className="text-sm text-zinc-500 dark:text-zinc-400 mb-3 truncate">
            {isLoaded && user
              ? user.emailAddresses?.[0]?.emailAddress || "—"
              : "Cargando..."}
          </div>

          {/* Workspace info con datos reales */}
          <div className="flex items-center gap-3 p-3 rounded-lg bg-zinc-50 dark:bg-zinc-800/50 mb-3">
            <Avatar className="h-8 w-8">
              <AvatarImage src={user?.imageUrl} alt={user?.fullName || ""} />
              <AvatarFallback className="text-xs font-bold">{initials}</AvatarFallback>
            </Avatar>
            <div className="flex-1 min-w-0">
              <div className="text-sm font-medium truncate">
                {isLoaded && user ? (user.fullName || "Personal") : "..."}
              </div>
              <div className="text-xs text-zinc-500 dark:text-zinc-400">Plan Free</div>
            </div>
            <Link
              href="/profile"
              onClick={() => setOpen(false)}
              className="text-blue-500 hover:text-blue-600 transition-colors"
            >
              <ChevronRight className="h-4 w-4" />
            </Link>
          </div>

          <div className="space-y-0.5">
            <Link
              href="/profile/settings"
              onClick={() => setOpen(false)}
              className="flex items-center gap-3 w-full px-3 py-2.5 text-sm text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors"
            >
              <Settings className="h-4 w-4 text-zinc-500" />
              <span>Configuración</span>
            </Link>

            <Link
              href="/profile/api-keys"
              onClick={() => setOpen(false)}
              className="flex items-center gap-3 w-full px-3 py-2.5 text-sm text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors"
            >
              <Key className="h-4 w-4 text-zinc-500" />
              <span>API Keys</span>
            </Link>
          </div>

          <div className="my-2 border-t border-zinc-200 dark:border-zinc-700" />

          <div className="space-y-0.5">
            <button className="flex items-center gap-3 w-full px-3 py-2.5 text-sm text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors">
              <Crown className="h-4 w-4 text-zinc-500" />
              <span>Mejorar plan</span>
              <ChevronRight className="h-4 w-4 ml-auto text-zinc-400" />
            </button>
          </div>

          <div className="my-2 border-t border-zinc-200 dark:border-zinc-700" />

          <button
            onClick={() => {
              setOpen(false)
              signOut({ redirectUrl: "/sign-in" })
            }}
            className="flex items-center gap-3 w-full px-3 py-2.5 text-sm text-left hover:bg-zinc-100 dark:hover:bg-zinc-800 rounded-lg transition-colors text-red-600 dark:text-red-400"
          >
            <LogOut className="h-4 w-4" />
            <span>Cerrar sesión</span>
          </button>
        </div>
      </PopoverContent>
    </Popover>
  )
}
